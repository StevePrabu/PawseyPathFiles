...
def getHeadPeaks(headTimeSteps, noradid, mwa, sat, ts, args):
    ...
    for t in headTimeSteps:
        hdu = fits.open(args.filePrefix + "-t" + str(t).zfill(4) + ".fits")
...
def getMidPoints(midTimeSteps, mwa, sat, ts, x_fit, y_fit, function, args):
    ...
    for t in midTimeSteps:
        hdu_head = fits.open(args.filePrefix + "-t" + str(t).zfill(4) + ".fits")
        hdu_tail = fits.open("Neg" + args.filePrefix + "-t" + str(t).zfill(4) + ".fits")
...
def main(args):
    ...
    hdu = fits.open(args.filePrefix + "-t" + str(headTimeSteps[0]).zfill(4) + ".fits")
