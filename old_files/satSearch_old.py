...
def getTLE(t):
    ...
    entries = 0
    print("TLE_catalog" + custom_name + ".txt")
...
def main(obs, t1, t2):
    ...
    for t in timeSteps:
        ...
        for satNo in tqdm(range(noObjects)):
            ...
            if line1 in sats_searched:
                continue

            else:

                sats_searched.append(line1)
                ...
                if visible:
                    detected = checkDetected(x, y , radius, data)
                    sat_x.append(x)
                    sat_y.append(y)
                    ...
                    if detected:
                        ...
                        if line1 in detectionSummary:

                            ## obtain  prev values
                            prev_timeSteps = detectionSummary[str(line1)]["timeSteps"]
                            prev_total = detectionSummary[str(line1)]["total"]
                            current_timeSteps = prev_timeSteps + "-" + str(t)
                            current_total = str(int(prev_total) + 1)

                            ## update values
                            detectionSummary[str(line1)]["timeSteps"] = current_timeSteps
                            detectionSummary[str(line1)]["total"] = current_total
                          

                        else:

                            entry = dict(timeSteps=str(t), total="1", norad=norad)
                            detectionSummary[str(line1)] = entry
