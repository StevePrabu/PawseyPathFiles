...
def getMidPoints(MidTimeSteps, noradid, mwa, sat, ts, x_fit, y_fit, function):
    ...
    for t in tqdm(midTimeSteps):
        ...
        UTCTime = datetime.strptime(hdu_head[0].header['DATE-OBS'][:-2], '%Y-%m-%dT%H:%M:%S')
...
def  getUTC(head_time, tail_time, mid_time):
    """
    ...
    return head_UTC, tail_UTC, mid_UTC
...
def main(obs, norad):
    ...
    head_UTC, tail_UTC, mid_UTC = getUTC(head_time, tail_time, mid_time)
    head_LST, tail_LST, mid_LST = getLST(head_UTC, tail_UTC, mid_UTC)

    ## save scatter points to file
    with open(str(norad) + "_extracted_test_data_from_" + str(obs) + ".csv", "w") as vsc:

        if debug:
            eprint("writing the obtain points to file...")        

        thewriter = csv.writer(vsc)
        thewriter.writerow(["x", "y", "ra", "dec", "UTC", "LST", "location", "t"])

        for x, y, ra, dec, time, lst, t in zip(head_x, head_y,head_ra, head_dec, head_UTC, head_LST, head_time):
            output = [x, y, ra, dec, time, lst, "head", t]
            thewriter.writerow(output)

        for x, y, ra, dec, time, lst, t in zip(tail_x, tail_y, tail_ra, tail_dec, tail_UTC, tail_LST, tail_time):
            output = [x, y, ra, dec, time, lst, "tail", t]
            thewriter.writerow(output)

        for x, y, ra, dec, time, lst, t in zip(mid_x, mid_y, mid_ra, mid_dec, mid_UTC, mid_LST, mid_time):
            output = [x, y, ra, dec, time, lst, "mid", t]
            thewriter.writerow(output)
