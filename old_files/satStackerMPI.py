...
def integrate(signal):
    ...
    return np.sum(filtered_data)/len(signal)
...
def main(args):
    ...
    hdu = fits.open(str(args.obs) + "-2m-1-0000-dirty.fits" )
    ...
    np.save("waterfall.npy", waterfall)
...
def main_old(args):
    ...
    for f in range(args.channels):
        ...
        for phase in tqdm(phase_array):
            ...
            ## apply low pass and integrate
            waterfall[phase_counter,f] = integrate(signal)
            phase_counter += 1
    plt.imshow(waterfall, origin="lower")
    plt.colorbar()
    plt.show()
