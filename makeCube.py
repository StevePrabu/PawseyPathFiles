...
def main(args):
    ...
    for f in tqdm(range(args.channels)):
        global_data = []
        for t in timeSteps:
            hduH = fits.open(str(args.obs) + "-" + str(args.midName) + "-" + str(t) + "h-" + str(f).zfill(4) + "-dirty.fits" )
            hduT = fits.open(str(args.obs) + "-" + str(args.midName) + "-" + str(t) + "t-" + str(f).zfill(4) + "-dirty.fits" )
